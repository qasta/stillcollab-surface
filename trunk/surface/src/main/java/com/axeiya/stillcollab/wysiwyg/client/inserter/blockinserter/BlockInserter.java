package com.axeiya.stillcollab.wysiwyg.client.inserter.blockinserter;
...
/**
 * Cet Inserter injecte un élément du type générique indiquée autour de la sélection courante
 * 
 * @author Damien Picard
 * 
 * @param <E> Type encapsulant
 */
abstract public class BlockInserter<E extends Element> extends Inserter {
  ...
  public void insert(final Selection selection) {
    ...
    if (startNode == null || endNode == null) {
      ...
      while (child != null && !finished) {
        if (started && range.containsNode(child, false)/* !child.isOrHasChild(endNode) */) {
          nextSibling = child.getNextSibling();
          ...
          if (!started && range.containsNode(child, true)/* child.isOrHasChild(startNode) */) {
            started = true;
            if (child.equals(startNode)) {
              ...
              if (child.getNodeType() == Node.TEXT_NODE) {
                ...
                Node copy = document.createTextNode(inner);
            ...
            child = nextSibling;
          } else if (started && range.containsNode(child, true)/* child.isOrHasChild(endNode) */) {
            finished = true;
   ...
   * 
   * @param selection Sélection courante
  ...
  /**
   * Permet de redresser si besoin le prédicat indiquant que l'élément sélectionné est du type
   * inséré
   * 
   * @param matchingAncestor Ancêtre commun dont le type correspond au type manipulé par cet
   *          Inserter
